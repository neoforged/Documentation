...
AT files can be specified anywhere as long as they are copied to the `resources` output directory on compilation. If no access transformer is defined within `neoforge.mods.toml`, NeoForge will search for `META-INF/accesstransformer.cfg`.
