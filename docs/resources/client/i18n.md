...
The values given in the JSON fles are mostly passed on as they are. The only exception are parameter codes, i.e. sequences that start with a "%". These will be evaluated and replaced with any parameters that are given to the translatable Component in the code.
...
To insert the content of anoter language key, use the tag `<ref:name>`, for example:
