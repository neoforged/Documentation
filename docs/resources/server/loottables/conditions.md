...
_See also: [Item Predicates][predicatejson] on the [Minecraft Wiki][mcwiki]_
...
This condition references a predicate file and returns its result. See [Item Predicates][predicate] for more information. Requires no loot parameters.
...
This condition accepts a [number provider][numberprovider] representing a chance between 0 and 1, and randomly returns true or false depending on that chance. The number provider should generally not return values outside the `[0, 1]` interval. Requires no loot parameters.
...
This condition accepts a [number provider][numberprovider] and an `IntRange`, returning true if the result of the number provided is within the range. Requires no loot parameters.
...
This condition checks if the world time is within an `IntRange`. Optionally, a `period` parameter can be provided to modulo the time with; this can be used to e.g. check the time of day if `period` is 24000 (one in-game day/night cycle has 24000 ticks). Requires no loot parameters.
...
This condition checks the current weather for raining and thundering. Requires no loot parameters.
...
This condition checks a given `EntityPredicate` against an entity target. The `EntityPredicate` can check the entity type, mob effects, nbt values, equipment, location etc. Requires no loot parameters.
...
This condition only returns true if the surrounding loot table id matches. This is typically used within [global loot modifiers][glm]. Requires no loot parameters.
...
:::tip
The vanilla loot condition types can be found in the `LootItemConditions` class.
:::
