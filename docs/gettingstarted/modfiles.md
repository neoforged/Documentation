...
```
 ...
 속성              |   타입    |           기본값           |                                                                                          설명                                                                                          | 예시                                                                                 
:----------------|:-------:|:-----------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------------------------------------
 `modId`         | string  |         **필수**          |                                  모드를 위한 고유 식별자입니다. 아이디는 다음 정규석에 매치되어야 합니다: `^[a-z][a-z0-9_]{1,63}$` (2~64 글자 사이, 모두 소문자를 사용해야 함, 숫자 또는 밑줄(_)을 허용함.)                                  | `"examplemod"`                                                                     |                                                       
 ...
 `displayName`   | string  |       `modId` 의 값       |                                                                       모드를 화면에 나타낼때 사용합니다. (예시. 모드 목록, 모드 불일치).                                                                       | `"예제 모드"`                                                                          |                                                                          
 `description`   | string  | `"MISSING DESCRIPTION"` |                                                             모드 목록 화면에 표시되는 모드의 설명입니다. [TOML에서 문자열 여러줄 작성하기][multiline].                                                              | `"이것은 예제 모드입니다!"`                                                                  |                                                                  
 `logoFile`      | string  |         *비어있음*          |                                     모드 목록 화면에서 사용되는 이미지 파일의 이름과 확장자입니다. 로고는 JAR의 루트에 있거나 소스 루트에 직접 있어야 합니다(예: 기본 소스 세트의 경우 `srcmainresources`).                                      | `"example_logo.png"`                                                               |                                                               
 ...
 `modUrl`        | string  |         *비어있음*          |                                                                         모드 다운로드 페이지지의 URL입니다. 현재 사용되지 않습니다.                                                                          | `"https://files.minecraftforge.net/"`                                              |
